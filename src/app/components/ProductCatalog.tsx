  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: isInCart ? "1px solid rgba(0,113,227,0.4)" : "1px solid rgba(0,113,227,0.12)",
        boxShadow: isInCart
          ? "0 0 28px rgba(0,113,227,0.12), 0 8px 32px rgba(0,0,0,0.4)"
    ...
    >
      {product.image && (
        <div className="relative w-full overflow-hidden" style={{ height: 210, background: "rgba(8,10,18,0.9)" }}>
          ...
          <div
            className="absolute inset-0"
            style={{ background: "linear-gradient(135deg, rgba(0,113,227,0.04) 0%, transparent 60%)" }}
          />
          {isInCart && (
            <motion.div
              initial={{ scale: 0 }}
      ...
      <div className="px-5 pb-5 -mt-1">
        <div className="mb-4">
          <h3
            className="font-black leading-none tracking-tight"
            style={{ fontSize: "2.6rem", letterSpacing: "-0.05em", color: "var(--tp-fg5)" }}
          >
            {product.name.toUpperCase()}
          </h3>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1 rounded-xl p-1" style={{ background: "rgba(15,18,30,0.9)" }}>
            <motion.button
              ...
              disabled={quantity === 0}
              className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 disabled:opacity-30"
              style={{ background: quantity > 0 ? "rgba(51,65,85,0.9)" : "transparent", color: "var(--tp-fg5)" }}
            >
              <Minus className="w-4 h-4" />
            </motion.button>
            <AnimatePresence mode="wait">
            ...
            <motion.button
              ...
              style={{
                background: "linear-gradient(135deg, rgba(0,113,227,0.25), rgba(0,91,181,0.2))",
          ...
          {isInCart && (
            <motion.div initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="text-right">
              <p className="text-xs" style={{ color: "var(--tp-fg3)" }}>Subtotal</p>
              <p className="font-bold text-sm" style={{ color: "var(--tp-fg5)" }}>${product.price * quantity}</p>
            </motion.div>
          )}
        </div>
      </div>
...
function VerdeCard({ product, quantity, onUpdateCart, index }: CardProps) {
  ...
  return (
    <motion.div
      ...
      style={{
        minHeight: 340,
        border: isInCart ? "1px solid rgba(0,113,227,0.35)" : "1px solid rgba(63,63,70,0.3)",
        boxShadow: isInCart
          ? "0 0 32px rgba(0,113,227,0.1), 0 12px 40px rgba(0,0,0,0.5)"
    ...
    >
      ...
      <div
        className="absolute inset-0"
        style={{ background: "linear-gradient(160deg, rgba(9,9,11,0.75) 0%, rgba(12,14,20,0.65) 40%, rgba(15,16,20,0.9) 80%, rgba(9,9,11,1) 100%)" }}
      />
      <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, rgba(0,113,227,0.05) 0%, transparent 50%)" }} />
      ...
      <div className="absolute top-4 right-4 w-24 h-24 opacity-70">
        <svg width="96" height="96" viewBox="0 0 96 96">
          ...
          <circle cx="48" cy="48" r="22" fill="none" stroke="rgba(125,196,255,0.15)" strokeWidth="0.8" />
          <circle cx="48" cy="48" r="36" fill="none" stroke="rgba(125,196,255,0.1)" strokeWidth="0.5" />
        </svg>
      </div>
      ...
      <div className="relative z-10 p-5 flex flex-col" style={{ minHeight: 340 }}>
        <div className="flex-1">
          <div className="mb-1">
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "rgba(125,196,255,0.6)" }}>
              Cepa Premium
            </span>
          </div>
          <h3
            ...
            style={{
              ...
              textShadow: "0 0 40px rgba(0,113,227,0.2)",
            }}
          >
            {product.name.toUpperCase()}
          </h3>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="font-black" style={{ fontSize: "3rem", letterSpacing: "-0.04em", color: "rgba(125,196,255,0.9)" }}>
              {product.price}
            </span>
            <span style={{ color: "rgba(125,196,255,0.5)", fontSize: "1rem", fontWeight: 600 }}>/ 3.5g</span>
          </div>
          <p className="mt-1 text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(125,196,255,0.4)" }}>
            Cosecha 2026
          </p>
          {isInCart && (
            <motion.div
            ...
            >
              <span className="text-xs font-bold" style={{ color: "rgba(125,196,255,0.9)" }}>
        ...
        <div className="mt-auto">
          <div className="w-full h-px mb-4" style={{ background: "linear-gradient(to right, rgba(125,196,255,0.15), transparent)" }} />
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold" style={{ color: "rgba(125,196,255,0.7)" }}>${product.price} / 3.5</p>
            <div className="flex items-center gap-1 rounded-xl p-1" style={{ background: "rgba(5,15,8,0.8)", border: "1px solid rgba(125,196,255,0.1)" }}>
              <motion.button
                whileTap={{ scale: 0.82 }}
                onClick={() => onUpdateCart(product.id, Math.max(0, quantity - 1))}
                disabled={quantity === 0}
                className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 disabled:opacity-30"
                style={{ background: quantity > 0 ? "rgba(39,39,42,0.9)" : "transparent", color: "var(--tp-accent-text)" }}
              >
                <Minus className="w-4 h-4" />
              </motion.button>
              <AnimatePresence mode="wait">
              ...
              <motion.button
                ...
                className="w-9 h-9 rounded-full flex items-center justify-center"
                style={{
                  background: "linear-gradient(135deg, rgba(0,113,227,0.3), rgba(0,91,181,0.2))",
          ...
          >
            <div className="max-w-lg mx-auto">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={onContinue}
                className="w-full py-4 px-6 rounded-3xl text-white font-semibold text-base flex items-center justify-between transition-all duration-300 glow-blue"
